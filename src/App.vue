<template>
  <nav>
    ...
    <div class="state-login" @click="triggerPop()">
      ...
      <img
        ...
        class="user"
        v-if="creds.name && creds.type == 'hotel'"
      />
      <img
        src="./assets/user.png"
        class="user"
        v-else-if="creds.name && creds.type == 'user'"
      />
  ...
  <div
    class="popup-module"
    v-if="popupFlag && creds.name"
    @click="triggerPop()"
  >
    <div class="popup">
      <ul>
        <li v-if="creds.type == 'hotel'">
          <router-link to="/mylistings">My Listings</router-link>
        </li>
        <li v-else-if="creds.type == 'user'">
...
export default {
  setup() {
    ...
    const logOut = ref(() => {
      store.commit("updateLogin", { name: "", email: "", type: "", id: 0 });
